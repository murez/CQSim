...
class Node_struc:
    nodeStruc: list[NodeStructure]
    job_list: list[JobInfo]
    predict_node: list[PredictNode]
    predict_job: list[PredictJob]

    def __init__(self, debug: Debug_log):
    ...
    def reset(self, debug: Debug_log):
    ...
    def node_allocate(self, proc_num: int, job_index: int, start: Time, end: Time):
        ...
        temp_job_info: JobInfo = {"job": job_index, "end": end, "node": proc_num}
        j = 0
        is_done = 0
    ...
    def pre_get_last(self):
        ...
        pre_info_last: dict[str, Time] = {"start": -1, "end": -1}
    ...
    def find_res_place(self, proc_num: int, index: int, time: Time):
